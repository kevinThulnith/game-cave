...
const Pong: React.FC = () => {
    ...
    const gameLoop = useCallback(() => {
        ...
        setBall(prevBall => {
            ...
            // Player paddle
            if (x <= PADDLE_WIDTH && y + BALL_SIZE >= playerY && y <= playerY + PADDLE_HEIGHT) {
                dx = -dx * 1.05; // Increase speed slightly
            }
            // AI paddle
            if (x >= GAME_WIDTH - PADDLE_WIDTH - BALL_SIZE && y + BALL_SIZE >= aiY && y <= aiY + PADDLE_HEIGHT) {
                dx = -dx * 1.05;
            }
            ...
            if (x < 0) { // AI scores
                newScore.ai++;
                resetBall(1);
            } else if (x > GAME_WIDTH) { // Player scores
                newScore.player++;
                resetBall(-1);
            }
    ...
    return (
        <div className="flex flex-col items-center">
            ...
            <div ref={gameAreaRef} className="relative bg-slate-900 border-2 border-slate-500 cursor-none" style={{ width: GAME_WIDTH, height: GAME_HEIGHT }}>
                {gameState !== 'playing' && (
                    <div className="absolute inset-0 flex flex-col justify-center items-center bg-black bg-opacity-70 z-10">
                        {gameState === 'start' ? (
                            <h3 className="text-4xl font-bold text-white mb-4">Welcome to Pong</h3>
                        ) : (
                            <h3 className="text-4xl font-bold text-white mb-4">{getMessage()}</h3>
                        )}
                ...
                <div className="absolute bg-cyan-400" style={{ width: BALL_SIZE, height: BALL_SIZE, left: ball.x, top: ball.y, borderRadius: '50%' }}></div>
                <div className="absolute bg-white" style={{ width: PADDLE_WIDTH, height: PADDLE_HEIGHT, left: 0, top: paddles.player }}></div>
                <div className="absolute bg-white" style={{ width: PADDLE_WIDTH, height: PADDLE_HEIGHT, right: 0, top: paddles.ai }}></div>
            </div>
