...
const BRICK_WIDTH = (GAME_WIDTH - BRICK_OFFSET_LEFT * 2 - BRICK_GAP * (BRICK_COLS - 1)) / BRICK_COLS;
...
const BrickBreaker: React.FC = () => {
    ...
    const gameLoop = useCallback(() => {
        ...
        // Wall collision
        if (newBall.x <= 0 || newBall.x >= GAME_WIDTH - BALL_RADIUS * 2) newBall.dx *= -1;
        if (newBall.y <= 0) newBall.dy *= -1;
        
        // Paddle collision
        if (newBall.y + BALL_RADIUS*2 >= GAME_HEIGHT - PADDLE_HEIGHT && newBall.x + BALL_RADIUS > paddleX && newBall.x < paddleX + PADDLE_WIDTH) {
            newBall.dy *= -1;
        ...
        newBricks.forEach(brick => {
            if (brick.alive) {
                if (newBall.x > brick.x && newBall.x < brick.x + brick.width && newBall.y > brick.y && newBall.y < brick.y + brick.height) {
                    newBall.dy *= -1;
                    brick.alive = false;
                    newScore += 10;
                }
    ...
    const brickColors = ['bg-red-500', 'bg-orange-500', 'bg-yellow-500', 'bg-green-500', 'bg-blue-500'];
    ...
    return (
        <div className="flex flex-col items-center">
            ...
            <div ref={gameAreaRef} className="relative bg-slate-900 border-2 border-slate-500" style={{ width: GAME_WIDTH, height: GAME_HEIGHT }}>
                 {gameState !== 'playing' && (
                    <div className="absolute inset-0 flex flex-col justify-center items-center bg-black bg-opacity-70 z-20">
                        ...
                        <button onClick={startGame} className="bg-violet-500 hover:bg-violet-600 text-white font-bold py-2 px-6 rounded-lg">
                            {gameState === 'start' ? 'Start Game' : 'Play Again'}
                        </button>
                    </div>
                )}
                {bricks.map((brick, i) => brick.alive && (
                    <div key={i} className={`absolute ${brickColors[i % BRICK_ROWS]}`} style={{ left: brick.x, top: brick.y, width: brick.width, height: brick.height }}></div>
                ))}
                <div className="absolute bg-cyan-400" style={{ left: paddleX, top: GAME_HEIGHT - PADDLE_HEIGHT, width: PADDLE_WIDTH, height: PADDLE_HEIGHT }}></div>
                <div className="absolute bg-white rounded-full" style={{ left: ball.x, top: ball.y, width: BALL_RADIUS*2, height: BALL_RADIUS*2 }}></div>
            </div>
             <p className="mt-4 text-slate-400">Move your mouse to control the paddle.</p>
        </div>
    );
};
